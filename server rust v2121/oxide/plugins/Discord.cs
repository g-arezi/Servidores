...
{
    [Info("Discord", "IMPERIALRUST", "1.0.0")]
    class Discord : RustPlugin
    {
        //Plugin made by Takashi
        //Dont alter the line codes with not (")

        bool configChanged;

        string tag            = "IMPERIALRUST";
        int sizeTag           = 15;
        string colorTag       = "red";
        string discordInvite  = "discord.gg/4vVy6x7";
        string mensagemAuto   = "Utilize <color=blue>/discord</color> para obter nosso servidor de <color=blue>discord</color>";
        int messageTime       = 120;
        ...
        void LoadConfigValues()
        {
            ...
            Puts("Configurações atualizadas! By Takashi");
            SaveConfig();
        }
        ...
        {
            Say(player, "Este é o link de convite do nosso servidor do <color=blue>discord</color>: <color=lightblue>" + discordInvite + "</color>");
        }
        ...
        void Say(BasePlayer player, string message)
        {
            ...
            Player.Message(player, $"<size={size}><color={color}>{tag}</color>: {message}</size>", null, 76561198811793305);
        }
        ...
        void Broadcast(string message)
        {
            var color = Convert.ToString(colorTag);
            var size = Convert.ToString(sizeTag);
            Server.Broadcast($"<size={size}><color={color}>{tag}</color>: {message}</size>", null, 76561198811793305);
        }
