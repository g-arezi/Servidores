...
namespace Oxide.Plugins
{
    ...
    public class AdminRadar: RustPlugin
    {
                ...
                try
                {
                    ...
                    foreach (var target in BasePlayer.activePlayerList.Where(target => target != null && target.transform != null && target.IsConnected))
                    {
                        ...
                        if (currDistance < playerDistance)
                        {
                            ...
                            if (drawText) player.SendConsoleCommand("ddraw.text", invokeTime + flickerDelay, color, target.transform.position + new Vector3(0f, 2f, 0f), string.Format("<size=10>{0} <color={1}>{2}</color> <color={3}>{4}</color>{5} {6}</size>", target.displayName ?? target.userID.ToString(), healthCC, Math.Floor(target.health), distCC, currDistance, vanished, extText));
        ...
        bool HasAccess(BasePlayer player)
        {
            ...
            if (permission.UserHasPermission(player.UserIDString, permName) && player.HasPlayerFlag(BasePlayer.PlayerFlags.IsAdmin))
                return true;
